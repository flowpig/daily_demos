...
class BaseSSHConnection(object):
    ...
    def __init__(self, ip=u'', host=u'', username=u'', password=u'', secret=u'', port=22,
                 ...
                 alt_key_file='', ssh_config_file=u'~/.ssh/config', timeout=60):
        ...
        if not ssh_strict:
            self.key_policy = DoNothingPolicy()
        else:
    ...
    def establish_connection(self):
        ...
        self.remote_conn_pre.set_missing_host_key_policy(DoNothingPolicy())
        ...
        try:
            ...
            msg = "Unable to connect {device_type} {ip}:{port}".format(
                device_type=self.device_type, ip=self.host, port=self.port)
            msg += '\n' + str(e)
            try:
    ...
    def send_command(self, command_string, delay_factor=.1, max_loops=150,
        ...
        if (command_string.find("logout") >= 0):
            return None
...
#                output += tmp_output
#            print(output);
#            if re.search(NEEDINPUT,output,re.MULTILINE):
#                output = re.sub(NEEDINPUT,"",output,0,re.MULTILINE)
#                self.remote_conn.sendall(" ");
#            else:
#                break;
#        
